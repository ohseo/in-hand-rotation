...
public class BBExperimentSceneManager : MonoBehaviour
{
    ...
    private const float TRIAL_START_POSITION_DELTA = 0.001f;
    private const float TRIAL_START_ROTATION_DELTA = 0.1f;
    ...
    void Update()
    {
        ...
        if (_awaitingTrialStart && _die != null)
        {
            float positionDelta = (_die.transform.position - _dieInitialPose.position).magnitude;
            Quaternion rotationDelta = _die.transform.rotation * Quaternion.Inverse(_dieInitialPose.rotation);
            rotationDelta.ToAngleAxis(out float rotationAngle, out _);
            if (rotationAngle > 180f) rotationAngle = 360f - rotationAngle;
            if (positionDelta > TRIAL_START_POSITION_DELTA || rotationAngle > TRIAL_START_ROTATION_DELTA)
            {
                _awaitingTrialStart = false;
                OnTrialStart?.Invoke();
            }
        }
